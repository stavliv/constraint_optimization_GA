...
% Mutation rate range to study
mutation_rates = [0.01, 0.05, 0.1, 0.2, 0.5]; % Different mutation rates to test
results_mutation = struct();
...
for m_idx = 1:length(mutation_rates)
    mutation_rate = mutation_rates(m_idx);
    fprintf('\nTesting Mutation Rate: %.2f\n', mutation_rate);
    ...
    % Store results for this mutation rate
    results_mutation(m_idx).mutation_rate = mutation_rate;
    results_mutation(m_idx).best_fitness_history = best_fitness_history;
    results_mutation(m_idx).avg_fitness_history = avg_fitness_history;
    results_mutation(m_idx).final_best_fitness = best_fitness;
    results_mutation(m_idx).final_best_solution = best_solution;
end

% Plot Results for Mutation Rate Study
figure;
hold on;
for m_idx = 1:length(mutation_rates)
    plot(1:num_generations, results_mutation(m_idx).best_fitness_history, 'LineWidth', 2, ...
        'DisplayName', sprintf('Mutation Rate %.2f', mutation_rates(m_idx)));
end
xlabel('Generation');
ylabel('Best Fitness');
title('Effect of Mutation Rate on Best Fitness');
legend('show');
grid on;
hold off;